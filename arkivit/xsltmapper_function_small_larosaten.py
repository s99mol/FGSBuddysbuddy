...
class XsltMaker:
    ...
    def createXslt(self, outputfolder):
        ...
        #anything = self.GUIvalues['-ANYTHING-']
        ...
        # Skapar anything
        #template_anything= etree.SubElement(rotelement, str(QName(ns.get('xsl'),'template')))
        #template_anything.set('match', 'row')
        
        #choose = etree.SubElement(template_anything, str(QName(ns.get('xsl'),'choose')))
        #when = etree.SubElement(choose, str(QName(ns.get('xsl'),'when')))
        
        ##Attachment = etree.SubElement(template_attachment, str(attachment))
        #Anything = etree.SubElement(when, str(anything))
        
        #for (k,v) in self.GUIvalues.items():
            
            
            #if k.startswith('-ANYTHING'):
            ...
            #if k.startswith('-ANYTHINGELSE'):
                #schema = v
                #attribute_anything = etree.SubElement(Anything, str(QName(ns.get('xsl'),'attribute')))
                #attribute_anything.set('name', schema)
                #valueof18 = etree.SubElement(attribute_anything, str(QName(ns.get('xsl'),'value-of')))
                #valueof18.set('select', infile)

        #Bevaras = etree.SubElement(Anything, str('Bevaras')).text = 'true'
