...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns="http://xml.ra.se/e-arkiv/FGS-ERMS">
    ...
    <xsl:key name="mode_3" match="row" use="concat(ArkivobjektID_Arende, '|', ArkivobjektID_Handling)"/>
    ...
    <xsl:template match="row" mode="mode_2">
        <ArkivobjektHandling>
            ...
            <xsl:apply-templates select="key('mode_2', concat(ArkivobjektID_Arende, '|', ArkivobjektID_Handling))" mode="repeat_mode_2"/>
        </ArkivobjektHandling>
    </xsl:template>
    <xsl:template match="row" mode="repeat_mode_2">
