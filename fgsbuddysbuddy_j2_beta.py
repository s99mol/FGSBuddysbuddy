...
Created on Sep 9 2023
 ...
 <FGS Buddysbuddy: Creates xslt file through mapping processes and uses that file to create xml metadata file.>
    ...
    This product includes software developed by Pallets (Jinja2)
    licensed under the BSD 3-Clause License. See the accompanying
    LICENSE-JINJA2.txt file for full license details.
...
def generate_xslt_with_jinja2(created_objects_data, ui_values, object_count, csv_headers, output_path):
    """Generates the final XSLT stylesheet using Jinja2 templates."""
    try:
        ...
        context = {
            "ui_values": ui_values,
            "created_objects_data": created_objects_data,
            "object_count": object_count,
            "csv_headers": csv_headers,
            "xmlns": ui_values.get('-XML_NAMESPACE-', ""),
            "root_element": ui_values.get('-ROOT_ELEMENT-', "Leveransobjekt"),
            "root_key_csv": root_key_csv,
            "sub_element_1": ui_values.get('-SUB_ELEMENT_1-'),
            "indent_space": '    '
        }

        template = env.get_template('stylesheet.j2')
        final_xslt_output = template.render(context)
...
while True:
    ...
    # --- Analyser Tab Events ---
       ...
       if event == '-SIMPLE_IS_HEADER-':
           ...
           success = generate_xslt_with_jinja2(created_objects, values, object_count, csv_headers_list, xslt_output_path)
